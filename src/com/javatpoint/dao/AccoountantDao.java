...
public class AccoountantDao {
	...
	public static int update(AccountantBean bean){
		...
		try{
			Connection con=DB.getCon();
			PreparedStatement ps=con.prepareStatement("update accountant set name=?,email=?,password=?,address=?,contact=? where id=?");
			ps.setString(1,bean.getName());
			ps.setString(2,bean.getEmail());
			ps.setString(3,bean.getPassword());
			ps.setString(4,bean.getAddress());
	...
	public static List<AccountantBean> getAllRecords(){
		...
		try{
			...
			ResultSet rs=ps.executeQuery();
			while(rs.next()){
				AccountantBean bean=new AccountantBean(null, null, null, null, null);
	...
	public static AccountantBean getRecordById(int id){
		AccountantBean bean=new AccountantBean(id, null, null, null, null, null);
		try{
			Connection con=DB.getCon();
