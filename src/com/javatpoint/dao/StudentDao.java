...
public class StudentDao {
	public static int save(StudentBean bean){
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("insert into student(name,email,gender,course,gender,paid,due,address,contact) values(?,?,?,?,?,?,?,?,?)");
	...
	public static int update(StudentBean bean){
		...
		try{
			Connection con=DB.getCon();
			PreparedStatement ps=con.prepareStatement("update student set name=?,email=?,gender=?,course=?,fee=?,paid=?,due=?,address=?,contact=? where rollno=?");
			ps.setString(1,bean.getName());
			ps.setString(2,bean.getEmail());
			ps.setString(3,bean.getGender());
			ps.setString(4,bean.getCourse());
	...
	public static List<StudentBean> getAllRecords(){
		...
		try{
			...
			while(rs.next()){
				StudentBean bean=new StudentBean(0, null, null, null, null, 0, 0, 0, null, null);
	...
	public static StudentBean getRecordByRollno(int rollno){
		StudentBean bean=new StudentBean(rollno, null, null, null, null, rollno, rollno, rollno, null, null);
		try{
			Connection con=DB.getCon();
